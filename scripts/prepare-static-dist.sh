...
find . -mindepth 1 -maxdepth 1 ! -name dist ! -name .git ! -name .github ! -name node_modules ! -name scripts ! -name .env ! -name .cursor -print0 |
  while IFS= read -r -d '' item; do
    base=$(basename "$item")
    case "$base" in
      server.js|package.json|package-lock.json|render.yaml|Dockerfile|Procfile|requirements.txt|runtime.txt|.python-version|.dockerignore) continue ;;
    esac
    cp -a "$item" "dist/$base"
  done

# Drop backend/admin-ish files from the static bundle if copied.
rm -f dist/server.js dist/package.json dist/package-lock.json dist/render.yaml 2>/dev/null || true
